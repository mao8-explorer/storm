...
class MPPIPolicy:
    ...
    def _check_collisions(self, rollouts = None,link_pos_batch = None,link_rot_batch = None, modify = False):

        threshold=0.45

        # last_time = time.time_ns()
        if modify:
            colls_value = self.scene_collision_checker(link_pos_batch = link_pos_batch,
                                                   link_rot_batch = link_rot_batch,
                                                   modify=modify, 
                                                   threshold=0.45,
                                                   by_link=True)

        else:
            colls_value = self.scene_collision_checker(qs = rollouts, threshold=0.45,by_link=True)

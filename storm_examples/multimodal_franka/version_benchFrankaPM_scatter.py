...
class MPCRobotController(FrankaEnvBase):
    ...
    def reinit(self, bounds):
        ...
        self.traj_log = {'position':[], 'velocity':[], 'acc':[] , 'des':[] , 
                         'weights':[], 'collision': [], 'ee_pos':[], 'cart_goal_pos':[]}
...
if __name__ == '__main__':
    ...
    def run_experiment(bounds, csv_filename):
        with open('./SDFcost_Franka/raw/'+ csv_filename, 'a', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=controller.fieldnames)
            if not f.tell():
                writer.writeheader()

            with open('./SDFcost_Franka/meanstd/'+ csv_filename, 'a', newline='') as var_file:
                var_writer = csv.DictWriter(var_file, fieldnames=controller.fieldnames)
                ...
                # 创建一个字典来存储 goal_w 和它对应的 coll_w 的值
                # goal_w_to_coll_w = {
                #     10: [80, 90 ,100, 120, 150],
                #     20: [130,140, 150, 180,  200, 220, 250],
                # }
                for goal_w in np.arange(10, 80.1, 10):
                    for coll_w in np.arange(80, 250, 10):
                        # if g_w not in [10, 20, 50, 80] or coll_w not in [80, 100, 150, 200, 250]:

                # # 使用嵌套的 for 循环来遍历 goal_w 和它对应的 coll_w 的值
                # for goal_w, coll_w_values in goal_w_to_coll_w.items(): 
                #     for coll_w in coll_w_values:
                        controller.rollout_fn.targetcost_judge_w = round(goal_w , 1)
                        controller.rollout_fn.collision_judge_w = round(coll_w , 1)

                        results = []
                        for i in range(6):
                            controller.reinit(bounds = bounds)
                            row  = controller.run()

                            row['goal_w'] = goal_w 
                            row['collision_w'] = coll_w
                            # img_name='PM_'+str(goal_w)+'_'+ str(coll_w)+'_'+str(i)+'_leftright_'
                            # controller.plot_traj(root_path = './SDFcost_Franka/' , img_name = img_name, plot_traj_multimodal= True)
                            # with open(img_name+'.pkl', 'wb') as fpkl:
                            #      pickle.dump(controller.traj_log, fpkl)
                            writer.writerow(row)
                            f.flush()  # 刷新缓冲
                            results.append(row)
    ...
    # bounds = [-0.30, 0.30]
    # run_experiment(bounds, './SDFcost_Franka/SDFcost_FrankaPMScatter_bound0.03_vel0.02.csv')

    bounds = [-0.40, 0.40]
    run_experiment(bounds, 'SDFcost_FrankaPMScatter_bound0.04.csv')
